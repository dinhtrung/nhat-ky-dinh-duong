...
Trong sheet chọn món có 2 chip nguồn: **Thư viện món** (208 món khẩu phần thực tế — mặc định) và **Bảng TT30/2026** (722 món theo *bảng thành phần dinh dưỡng món ăn*, Phụ lục II Thông tư 30/2026/TT-BYT, hiệu lực 10/07/2026).
...
- Mỗi món giữ đủ **14 chỉ tiêu** của bảng gốc (năng lượng, đạm, béo, bột đường, xơ, canxi, sắt, kẽm, natri, kali, vitamin A, beta-caroten, vitamin C, cholesterol) — app đang theo dõi 4 chỉ tiêu đầu, phần còn lại lưu sẵn trong `data/tt30.json`.
- **64 món** trong bảng gốc không công bố đủ đạm/béo/bột đường → chỉ tiêu thiếu được suy từ năng lượng và đánh dấu `est: true` (hiển thị kèm `(≈)`).
- Khớp tên món: gõ không dấu như mọi chỗ khác (`mien luon`, `van than`, `cha ca`) — tên gốc được giữ nguyên, kể cả tên có số lượng như *Bánh bột lọc (5 cái)*.

## 📷 Chụp hoá đơn (OCR)

Đi ăn nhà hàng: vào luồng thêm món → bấm **nút 📷 cạnh ô tìm kiếm** (hoặc nút 📷 trong sheet khẩu phần) → app đọc bill, tách ra từng dòng món, đoán số lượng, khớp với thư viện khẩu phần (208 món) → **màn xem lại** để sửa/đổi món/bỏ dòng → bấm *Ghi N món* là xong cả bữa.
...
data/foods.json     thư viện món dựng sẵn (208 món, khẩu phần thực tế: 1 tô/1 bát/1 ổ…)
