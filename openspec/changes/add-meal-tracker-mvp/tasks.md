...
- [ ] 1.1 Tạo `~/nhat-ky-dinh-duong/`: `index.html`, `styles.css`, `app.js`, `manifest.json`, `service-worker.js`, `data/`, `icon-192.png`, `icon-512.png`, `README.md`, `CHANGELOG.md`, `.gitignore` (chặn `_seed.js`, `nhat-ky-dinh-duong-*.json`, `*.csv`)
...
- [ ] 1.4 Viết `service-worker.js` cache-first với `const CACHE = 'nhat-ky-dinh-duong-v1'`, cache `index.html`, `styles.css`, `app.js`, `manifest.json`, `data/foods.json`; thêm `skipWaiting` + `clients.claim`
...
- [ ] 2.3 UI thư viện: danh sách theo nhóm món (không chia vùng miền) + ô tìm kiếm trả kết quả khi gõ; ghim nhóm "Yêu thích" ở đầu
...
- [ ] 2.6 Hàm `getQuickPicks(limit=8)`: đếm tần suất `foodId` trong `meal_log` 30 ngày gần nhất → sắp xếp yêu thích trước → số lần chọn giảm dần → đồng hạng thì lần dùng gần nhất mới hơn; tính lại mỗi lần render, KHÔNG lưu bảng đếm riêng
...
- [ ] 4.9 Dải quick pick ở tab Hôm nay: hiển thị ~8 chip món hay ăn (từ `getQuickPicks`), 1 chạm là ghi ngay 1 khẩu phần vào nhóm bữa suy ra từ giờ hiện tại (trước 10h Sáng · 10–14h Trưa · 14–17h Chiều · 17–22h Tối · còn lại Ăn vặt), kèm nút đổi nhóm bữa của mục vừa thêm (không cần xoá/tạo lại)

## 5. Dữ liệu của người dùng

- [ ] 5.1 Xuất JSON toàn bộ (`meal_log` + `meal_config` + `meal_foods`) qua Blob + `a.download`, tên file `nhat-ky-dinh-duong-YYYY-MM-DD.json`
...
- [ ] 6.4 Bump `CACHE = 'nhat-ky-dinh-duong-v2'`, cập nhật `CHANGELOG.md` (mục `## v1.0.0 — 2026-09-09` + các nhóm ✨ Mới / 🎨 UI) và README (mô tả + cách cài)
